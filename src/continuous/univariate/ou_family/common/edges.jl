...
function _ou_stationary_design_node_means(
    ...
    # This is the exact deterministic OU path mean with root mean zero.
    # It satisfies m_child = edge_a * m_parent + edge_b.
    for node in tree.preorder
        tree.is_tip[node] && continue
        for edge in tree.first_child_edge[node]:tree.last_child_edge[node]
            edge == 0 && continue
            child = Int(tree.child_of_edge[edge])
            affine = _ou_edge_affine(cache, edge, spec, bundle)
            means[child] = affine.a * means[node] + affine.b
        end
    end
    return means
end
