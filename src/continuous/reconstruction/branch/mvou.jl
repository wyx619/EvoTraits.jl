function _mvou_segments_kernel(
    ...
    if model === :mvOUM || model === :mvOU1
        precalc = MVOUPrecalc(branch_lengths = Float64[sum(seg.length for seg in segments)])
        branch = _mvou_branch_cache(precalc, bundle.A, bundle.Sigma)
        return (Phi = branch.Phi[:, :, 1], Q = branch.Q[:, :, 1])
    elseif model === :mvOUMV
        precalc = MVOUPrecalc(branch_lengths = Float64[sum(seg.length for seg in segments)], edge_segments = [segments], nregimes = size(bundle.Sigma_regimes, 3))
        branch = _mvoumv_branch_cache(precalc, bundle.A, bundle.Sigma_regimes)
        return (Phi = branch.Phi[:, :, 1], Q = branch.Q[:, :, 1])
    elseif model === :mvOUMA
        precalc = MVOUPrecalc(branch_lengths = Float64[sum(seg.length for seg in segments)], edge_segments = [segments], nregimes = size(bundle.A_regimes, 3))
...
function _mvou_single_segment_kernel(model::Symbol, bundle::MVOUParameterBundle, seg::SimmapSegment, p::Integer)
    seg.length <= 0.0 && return _mvou_empty_kernel(p)
    return _mvou_segments_kernel(model, bundle, SimmapSegment[seg])
end
...
function _mvou_edge_kernel_cache(
    ...
    for i in 1:nseg
        segment_kernel[i] = _mvou_single_segment_kernel(model, bundle, segments[i], p)
